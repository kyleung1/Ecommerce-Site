...
const ProductDetails = ({ item }) => {
    ...
    const handleDelete = async () => {
        ...
        const response = await fetch('http://localhost:' + process.env.REACT_APP_PORT + '/item/' + item._id, {
            method: 'DELETE',
            headers: {
    ...
    return (
        <div className = "product-details">
            ...
            <img src = {require('../images/' + item.name + '.webp')} className = "item_images"/>
            <br></br>
            <p className = "product-desription">{item.desc}</p>
            {user && user.admin === true && <button onClick= {handleDelete}>delete</button>}
